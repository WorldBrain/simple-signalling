...
export class FirebaseSignalTransport implements SignalTransport {
    ...
    async allocateChannel(): Promise<{ initialMessage: string }> {
        ...
        const ref = this.options.database
            .ref(this.options.collectionName)
            .push(channelData)
    ...
    async openChannel(options: {
        ...
        if (!options.initialMessage) {
            throw new Error(
                `Opening a channel without an initial message is not supported yet`,
            )
        }

        const [key] = options.initialMessage.split(':')
        const channelRef = this.options.database
            .ref(this.options.collectionName)
            .child(key)
        await channelRef
            .child('created')
            .set(firebase.database.ServerValue.TIMESTAMP)
        return new FirebaseSignalChannel({
            channelRef,
...
export class FirebaseSignalChannel implements SignalChannel {
    ...
    constructor(
        private options: {
            channelRef: firebase.database.Reference
            deviceId: SignalDeviceId
        },
    ) {
        this.receiverQueueRef = options.channelRef.child(
            `${getReceiverDeviceId(options.deviceId)}Queue`,
        )
    }

    async connect() {
        this.options.channelRef
            .child(`${this.options.deviceId}Queue`)
            .on('child_added', this._processMessage)
    }
...
export function getSignallingRules() {
    const queueRules = {
        $message: {
            '.validate':
                'data.val() == null && newData.isString() && newData.val().length < 10240',
        },
    }
    ...
    return {
        $id: {
            ...
            '.indexOn': ['created'],
            '.validate':
                "data.child('created').val() != null || newData.child('created').val() == now",
